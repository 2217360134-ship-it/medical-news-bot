...
def fetch_news_node(state: FetchNewsInput, config: RunnableConfig, runtime: Runtime[Context]) -> FetchNewsOutput:
    """
    title: 获取指定来源新闻
    desc: 从今日头条、搜狐、腾讯网、网易新闻、凤凰网等获取医疗器械和医美相关的新闻
    integrations: 联网搜索
    """
    ctx = runtime.context
    ...
    # 从输入中获取目标新闻来源域名（最多支持5个）
    target_sites = state.target_sites or "toutiao.com|sohu.com|qq.com|163.com|ifeng.com"
    print(f"使用目标网站: {target_sites}")
