...
from graphs.state import (
    LoopGlobalState,
    FetchBatchInput, FetchBatchOutput,
    DeduplicateBatchInput, DeduplicateBatchOutput,
    AccumulateInput, AccumulateOutput,
    CheckThresholdInput
)
...
def fetch_batch_node(state: FetchBatchInput, config: RunnableConfig, runtime: Runtime[Context]) -> FetchBatchOutput:
    """
    title: 批次搜索新闻
    ...
    try:
        ...
        return FetchBatchOutput(batch_news_list=final_news)
...
def deduplicate_batch_node(state: DeduplicateBatchInput, config: RunnableConfig, runtime: Runtime[Context]) -> DeduplicateBatchOutput:
    """
    title: 批次去重
    ...
    print("=" * 60)
    print(f"[循环搜索] 批次去重")
    print(f"  当前批次: {len(state.batch_news_list)} 条")
    ...
    for news in state.batch_news_list:
        # 检查URL是否已存在
    ...
    return DeduplicateBatchOutput(deduplicated_batch_news=deduplicated_news)


def accumulate_node(state: AccumulateInput, config: RunnableConfig, runtime: Runtime[Context]) -> AccumulateOutput:
    """
    title: 累积新闻
    ...
    print("=" * 60)
    print(f"[循环搜索] 累积新闻")
    print(f"  新增: {len(state.deduplicated_batch_news)} 条")
    print(f"  累积前: {len(state.accumulated_news)} 条")
    
    # 累积新闻
    new_accumulated = state.accumulated_news + state.deduplicated_batch_news
...
def check_threshold(state: CheckThresholdInput) -> str:
    """
    title: 检查是否达到目标
